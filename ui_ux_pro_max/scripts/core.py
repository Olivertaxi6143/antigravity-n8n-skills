...
class BM25:
    ...
    def tokenize(self, text):
        """Lowercase, split, remove punctuation, filter short words (>=2 chars)"""
        ...
        for idx, doc in enumerate(self.corpus):
            ...
            for token in query_tokens:
                if token in self.idf:
                    ...
                    # Prevent division by zero if avgdl is 0 (empty corpus)
                    avgdl_safe = self.avgdl if self.avgdl > 0 else 1
                    denominator = tf + self.k1 * (1 - self.b + self.b * doc_len / avgdl_safe)
