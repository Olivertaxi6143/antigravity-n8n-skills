...
def format_ascii_box(design_system: dict) -> str:
    ...
    def wrap_text(text: str, prefix: str, width: int) -> list:
        """Wrap long text into multiple lines, handling words longer than width."""
        if not text:
            return []
        
        content_width = width - len(prefix) - 2 # 2 for the right border and padding
        if content_width <= 0: return [prefix + text[:5]] 
        ...
        for word in words:
            # If a single word is wider than content_width, split it
            while len(word) > content_width:
                part = word[:content_width]
                word = word[content_width:]
                if current_line != prefix:
                    lines.append(current_line.ljust(width) + "|")
                    current_line = prefix
                lines.append((prefix + part).ljust(width) + "|")
    ...
    if style.get("keywords"):
        lines.extend(wrap_text(f"Keywords: {style.get('keywords', '')}", "|     ", BOX_WIDTH))
    if style.get("best_for"):
        lines.extend(wrap_text(f"Best For: {style.get('best_for', '')}", "|     ", BOX_WIDTH))
    ...
    if colors.get("notes"):
        lines.extend(wrap_text(f"Notes: {colors.get('notes', '')}", "|     ", BOX_WIDTH))
    lines.append("|" + " " * BOX_WIDTH + "|")

    # Typography section
    lines.append(f"|  TYPOGRAPHY: {typography.get('heading', '')} / {typography.get('body', '')}".ljust(BOX_WIDTH) + "|")
    if typography.get("mood"):
        lines.extend(wrap_text(f"Mood: {typography.get('mood', '')}", "|     ", BOX_WIDTH))
    if typography.get("best_for"):
        lines.extend(wrap_text(f"Best For: {typography.get('best_for', '')}", "|     ", BOX_WIDTH))
    ...
    if effects:
        ...
        lines.extend(wrap_text(effects, "|     ", BOX_WIDTH))
        lines.append("|" + " " * BOX_WIDTH + "|")

    # Anti-patterns section
    if anti_patterns:
        lines.append("|  AVOID (Anti-patterns):".ljust(BOX_WIDTH) + "|")
        lines.extend(wrap_text(anti_patterns, "|     ", BOX_WIDTH))
        lines.append("|" + " " * BOX_WIDTH + "|")
